...
Доработать игру MineSweeper с помощью пользовательских событий:

1. Событие "mine.start" возникает при старте игры игроков (первый ход на чистом поле).

2. Событие "mine.step" возникает при очередном ходе игрока (передает координаты хода как дополнительный параметр в detail).

3. Событие "mine.end" возникает при завершении игры (передает результат игры как дополнительный параметр в detail).

Для каждого события должен быть обработчик, который выполняет логику игры.

По желанию можно предусмотреть дополнительные события.
